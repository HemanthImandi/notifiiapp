...
public class LogPackageOutAdapter extends RecyclerView.Adapter<LogPackageOutAdapter.ViewHolder> {

    public boolean mIsShowAddress1;
    private List<Package> mList;
    private ArrayList<Package> mOrinalList;
    ...
    public LogPackageOutAdapter(Activity context, List<Package> list, String timeZone) {
        ...
        mMailroomList.clear();
        if (SingleTon.getInstance().getmMailRoomList(context)!=null){
            mMailroomList.addAll(SingleTon.getInstance().getmMailRoomList(context));
        }
        mOrinalList = new ArrayList<>();
    }
    ...
    private void handleItemView(ViewHolder holder, int position) {
        ...
        } else if (noOfVisibleViews == 1) {
            holder.firstDynamicLL.setVisibility(View.VISIBLE);
            holder.secondDynamicLL.setVisibility(View.GONE);
            holder.secondLabelTV.setText("");
            holder.secondValueTV.setText("");
        } else if (noOfVisibleViews == 2) {
            holder.firstDynamicLL.setVisibility(View.VISIBLE);
            holder.secondDynamicLL.setVisibility(View.GONE);
        } else if (noOfVisibleViews == 3) {
            holder.firstDynamicLL.setVisibility(View.VISIBLE);
            holder.secondDynamicLL.setVisibility(View.VISIBLE);
            holder.fourthLabelTV.setText("");
            holder.fourthValueTV.setText("");
        } else {
            holder.firstDynamicLL.setVisibility(View.VISIBLE);
            holder.secondDynamicLL.setVisibility(View.VISIBLE);
        }
        if (mIsShowMailRoom) {
            ...
            holder.tvMailroom.setText(!item.getMailroomId().equalsIgnoreCase("null") ? " " + NTF_Utils.getMailroomName(item.getMailroomId(),mMailroomList) : "");
        } else {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        void onEditPkgClicked() {
            ...
            fragment.onKeyBoardStateChanged(false);
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            for (Package wp : mOrinalList) {
                wp.setIsChecked(false);
            }
            mList.addAll(mOrinalList);
        } else {
            for (Package wp : mOrinalList) {
                wp.setIsChecked(false);
                if (wp.getRecipientName().toLowerCase(Locale.getDefault()).contains(charText) ||
                        wp.getRecipientAddress1().toLowerCase(Locale.getDefault()).contains(charText) ||
                        wp.getTrackingNumber().toLowerCase(Locale.getDefault()).contains(charText) ||
                        wp.getTagNumber().toLowerCase(Locale.getDefault()).contains(charText)) {
                    mList.add(wp);
                }
            }
        }
        fragment.internalSearchCompleted(charText.length() > 1 && mList.size() > 1);
        notifyDataSetChanged();
    }

    public void setOriginalList(ArrayList<Package> list) {
        mOrinalList = new ArrayList<>();
        mOrinalList.addAll(list);
        System.gc();
    }

    public ArrayList<Package> getmOrinalList() {
        return mOrinalList;
    }
